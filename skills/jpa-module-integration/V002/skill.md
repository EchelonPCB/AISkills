...
| RALPH     | PASS after repo validation, Jetson-target verification, and handoff are complete |

---

# Objective

Take a set of frozen validated base modules and newly delivered contract Python files, analyze how they connect in the full sensing and control stack, restructure the repository so the entry point runs cleanly, and rewrite the integration loop with the correct fusion doctrine, PID steering, and hardware constants. Produce a syntax-verified, Jetson-target import-chain-verified demoday.py and the SCP commands needed to deploy it to the physical Jetson hardware.
...
- The request is a general code review unrelated to JPA runtime integration, Jetson deployment, or hardware validation.
...
3. Project context: `CLAUDE.md` and `.aigst/PROJECT.md` from the JPA workspace.
...
4. Import chain verification confirming all four runtime imports resolve on the Jetson and core logic checks pass.
...
4.1 Read `CLAUDE.md` and `.aigst/PROJECT.md` for workspace boundaries and engine relationship rules.
...
9.2 If running on a non-Jetson development host, do not treat missing `cv2`, `numpy`, or `smbus2` as a code defect. Use syntax checks and, if needed, mocked dependency imports only to verify path wiring and pure logic.
9.3 On the Jetson, run the real import chain check: insert `ssunc_perception/` into `sys.path`, import all four modules with real `cv2`, `numpy`, and `smbus2`, instantiate each class, verify `STEER_CENTER_US == 1563`, verify `us_to_ticks()` for center and both limits, verify `pid.compute(0.0) == 1563`, verify `pid.compute(None) == 1563`, verify `restrictive()` for all key cases.
9.4 On the Jetson, confirm physical interfaces before motion: camera opens, PCA9685 is visible on I2C bus 1 at address 0x40, V2I receiver binds UDP port 5005, and safe_stop centers steering and neutralizes throttle.
9.5 If any Jetson-target check fails, fix the defect before proceeding.
...
| Non-Jetson host lacks `cv2`, `numpy`, or `smbus2` | Record as environment mismatch; run real import verification on the Jetson |
| Jetson has the physical hardware connected | Run real dependency, camera, I2C, V2I, and safe_stop checks before motion |
...
2. Non-Jetson host validation, if used, is limited to syntax, path wiring, and pure logic; missing Jetson-only dependencies are not counted as code failures.
3. On the Jetson, all four module imports resolve from the relative ssunc_perception/ path with real `cv2`, `numpy`, and `smbus2`.
4. The Jetson camera opens through the selected capture pipeline.
5. The PCA9685 is reachable on I2C bus 1 at address 0x40 and prescale readback is correct.
6. V2I receiver binds UDP port 5005.
7. `safe_stop()` commands neutral throttle and centered steering on the physical hardware.
8. `us_to_ticks(1563) == 320`, `us_to_ticks(1367) == 280`, `us_to_ticks(1807) == 370`.
9. `pid.compute(0.0) == 1563` and `pid.compute(None) == 1563`.
10. `restrictive('GREEN', 'RED') == 'RED'` and `restrictive('UNKNOWN', 'RED') == 'RED'`.
11. No reference to `steer_center.txt` remains in `demoday.py`.
12. `sys.path` uses a relative path from `__file__`, not a hardcoded absolute Jetson path.
13. Fusion doctrine matches the user-confirmed behavior: AGREE-first when V2I is live, camera fallback when offline.
14. `pid.reset()` is called on every non-GREEN state.
15. CSV log columns include `lane_offset` and `steer_ticks`.

---

# RALPH Loop

Run at most two RALPH passes before declaring the integration complete:

- **R — Reaffirm Requirements:** Confirm contract files, frozen base files, JPA workspace context, fusion doctrine, hardware constants, and Jetson path layout.
- **A — Audit Alignment:** Confirm demoday.py behavior matches AGREE-first fusion, PID steering, hardware unit conversion, and safe-stop behavior.
- **L — Lock Layout and Lineage:** Confirm the runtime layout is `demoday.py` plus `ssunc_perception/`, frozen 0409 files remain unchanged, and AISkills/GSD/AIGST boundaries are preserved.
- **P — Purge Provenance Problems:** Remove stale CTS notes, hidden-chat assumptions, legacy absolute paths, and file dependencies such as `steer_center.txt`.
- **H — Halt or Hand Off:** PASS only after Jetson-target verification is complete; otherwise hand off with exact failing check and next command.
...
- Do not treat missing `cv2`, `numpy`, or `smbus2` on a non-Jetson development host as a JPA integration failure; the authoritative runtime check is on the Jetson.
...
| Development host cannot import `cv2`, `numpy`, or `smbus2` | Run syntax and mocked wiring checks locally, then run real import and hardware checks on the Jetson |
| Jetson camera or I2C hardware is unavailable | Stop before motion; repair hardware/service state and rerun Jetson verification |
...
- Jetson runtime with Python 3.8+ and real `cv2`, `numpy`, `smbus2` dependencies installed.
- Non-Jetson development host may be used for syntax and mocked wiring checks only.
...
- This skill is listed in `MANIFEST.md` and mirrored in `system/indexes/skill-index.json`.
...
## V002
- Bumped from V001 to preserve the original CTS output and add Jetson hardware-target validation, non-Jetson validation limits, `.aigst/PROJECT.md` context, and RALPH handoff gates.
