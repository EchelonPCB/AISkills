...
name: "chat-to-skill"
description: "Route supplied chats into classification mode and return complete, validated AISkills skill files when repeatable workflows exist."
...
| Field       | Detail                                                                 |
|-------------|------------------------------------------------------------------------|
| Trigger     | Skill file is supplied to classify a chat or extract a workflow        |
| Input       | Current chat, raw transcript, existing skills, scaffold metadata       |
| Output      | Classification, optional skill.md, changelog note, support-file plan   |
| Key Steps   | Lock routing -> classify -> extract -> generalize -> validate          |
...
Convert completed AI conversations into governed AISkills artifacts. When this skill is supplied as operating instructions, immediately classify the supplied or current chat before giving advice about the chat's subject matter. Determine whether the chat contains a reusable workflow, an amendment to an existing skill, a reference, an asset, a script, or insufficient material. For valid workflows, produce one complete, self-contained, copy-pastable `skill.md` file plus repo-ready metadata, a changelog note, and a support-file plan that fits the AISkills repository structure.
...
- A user uploads, pastes, or points to this skill file and expects the current chat to be classified
- A user corrects the assistant for giving advice instead of applying the chat-to-skill procedure
...
10. For non-qualifying chats, a clear classification result and reason instead of unrelated advice

# First Response Contract

When this skill is supplied as operating instructions and any source chat is available, the first substantive response must begin with this classification block:
...
If the classification is new skill or amendment, follow the block with `Skill File` and exactly one complete fenced markdown file. If the classification is reference, asset, script, split, insufficient, or reject, do not generate a skill file; return the classification, reason, missing information or support destination, and stop.

Do not answer the source chat's original subject-matter request before completing this classification block.
...
1.1 Immediately decide whether this file is being used as operating instructions.
1.2 IF the user uploads, pastes, quotes, or points to this skill and there is a current chat or supplied transcript, THEN choose **apply mode**.
1.3 IF the user provides chat history, says "use this chat to skill", asks to extract a skill, or asks what skill comes from the chat, THEN choose **apply mode**.
1.4 IF no separate transcript is supplied, THEN treat the current visible conversation as the source chat.
1.5 IF the user explicitly asks only to review, rate, amend, or improve `chat-to-skill`, THEN choose **review mode**.
1.6 IF both extraction and review are requested, THEN perform apply mode first and review mode second.
1.7 In apply mode, do not critique or improve `chat-to-skill` until after returning the classification and proposed skill artifact or non-qualifying result.
1.8 In review mode, do not create a new skill unless the user also supplied chat content and asked for extraction.
1.9 IF the user supplies this skill text plus a source chat, THEN treat this skill text as operating instructions and the source chat as the material to classify.
1.10 IF the user complains that a prior response gave advice, continued the live topic, or reviewed this skill instead of extracting, THEN restart in apply mode and return the classification block first.
1.11 Lock routing once apply mode is selected; do not switch into advisory mode until classification output is complete.

## 2. Intake

2.1 Separate operating instructions from source material.
2.2 If no separate transcript is attached, use the current visible conversation as source material.
2.3 Read the source chat content from start to finish.
2.4 Identify the main thing the source chat accomplished.
2.5 Identify all artifacts produced by the source chat, including documents, scripts, prompts, decisions, files, and operating rules.
2.6 Identify the minimum inputs needed to reproduce the same kind of outcome.
2.7 Identify any explicit user constraints that shaped the process.
2.8 If the source chat is too long or mixed, segment it into candidate input -> process -> output chains.
...
4.10 Treat non-qualification as a valid result; do not compensate by giving general advice about the source topic.
4.11 If the chat does not qualify, return the classification block and stop unless the user explicitly asks for next-step advice.
...
9.10 For reference, asset, script, insufficient, or reject classifications, return no `Skill File` heading and no skill frontmatter.
9.11 For non-qualifying classifications, output only classification, confidence, rationale, repo action, and any missing information or support destination.
...
11.15 Verify the first substantive response in apply mode starts with `Classification:`.
11.16 Verify no subject-matter advice appears before classification.
11.17 If any check fails, revise before returning output.
...
- IF this skill is supplied as operating instructions and a current chat exists, THEN choose apply mode by default.
- IF no separate transcript is supplied, THEN classify the current visible conversation.
- IF the chat does not qualify, THEN return the classification block and stop.
- IF advisory-mode drift occurs, THEN restart at the first response contract.
...
13. Apply-mode responses begin with classification before subject-matter advice.
14. Non-qualifying chats return a clean reject, insufficient, reference, asset, or script result instead of advice.
...
- Do not continue the source chat's subject matter after this skill is supplied until classification is complete.
- Do not treat a non-qualifying chat as permission to return generic advice.
...
- Advisory-mode drift after skill is supplied: stop, restart with the classification block, and classify the current visible chat.
- Non-qualifying chat produces advice instead of classification: replace the advice with reject, insufficient, reference, asset, or script classification.
...
## V009
- Added a first-response classification contract for apply mode.
- Added routing lock rules so uploaded or pasted skill instructions classify the current chat before advice.
