...
| Section | Description |
|--------|-------------|
| Objective | Define infographic workflow purpose |
| Trigger | When to apply the skill |
| Do Not Use When | Misuse conditions |
| Required Inputs | Minimum content and style inputs |
| Optional Inputs | Helpful audience and format context |
| Outputs | Expected concept and prompt deliverables |
| Support Layers | Reference and asset surfaces |
| Procedure | Stepwise infographic workflow |
| Decision Logic | Branch and approval decisions |
| Validation | Output quality checks |
| Rules | Non-negotiable constraints |
| Failure Modes | Common breakdowns |
| Dependencies | Required source and optional tools |
...
- The user only wants proofreading or plain text editing.
- The request is for a full design file without content or brand direction.
- The user wants actual image generation before a prompt or concept is approved.
- The content is too vague to identify a core message.
...
1. Intake summary
2. At least 5 infographic concept options
3. At least 5 visualization options for the selected concept
4. Brand-aligned final image-generation prompt
5. Optional negative prompt or exclusion list
6. Revision notes when the user requests iteration

# Support Layers

- Content layer: user-provided notes, posts, articles, or concepts.
- Brand layer: optional guidance in `references/`.
- Asset layer: optional examples or visual references in `assets/`.
- Approval layer: human-in-the-loop gates before final prompt or image generation.
...
| Condition | Action |
|----------|--------|
| content is missing | request content before generating concepts |
| brand references are missing | proceed with labeled assumptions |
| user wants autonomous continuation | continue through approval gates only as authorized |
| user rejects concept or direction | revise from the last approved stage |
| image generation is requested | build and approve prompt before generating |

# Validation

The infographic output is valid when:

1. Core message is clear.
2. Concept options include multiple viable directions.
3. Visualization options describe layout, hierarchy, flow, and density.
4. Final prompt includes subject, layout, hierarchy, style, platform, and exclusions.
5. Missing references are disclosed rather than invented.
...
## V003
- Added governed Index, Do Not Use When, Outputs, Support Layers, Decision Logic, Validation, and Change Log sections.
- Preserved V002 infographic concept, visualization, prompt, and approval workflow.
