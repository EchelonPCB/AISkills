...
4. Repo context such as `MANIFEST.md`, `system/indexes/skill-index.json`, or the selected live skill when writing directly into AISkills
...
- Discovery layer: uses repo context to distinguish a new skill from an amendment when AISkills is available.
...
2.4 If repo context exists, inspect `MANIFEST.md`, `system/indexes/skill-index.json`, or a selected live skill before finalizing identity.
2.5 If a matching live skill already exists and the user wants a repo-backed update, classify the result as an amendment and bump the existing skill instead of overwriting it.
2.6 Ask one concise question only when a missing answer changes skill identity, authority, target runtime, or destructive scope.
...
| matching live skill exists and repo-backed write is requested | classify as amendment and bump the live skill before editing |
| missing fact changes skill identity, authority, target runtime, or destructive scope | ask one concise question before output |
...
- Do not overwrite an existing live skill when the contract maps to an existing governed workflow; bump it first in repo-backed mode.
- Do not ask broad exploratory questions. Ask at most one concise question unless the user explicitly requests a deeper design session.
- Do not generate companion references, scripts, or assets unless the user explicitly requests them or the contract requires them.
- Do not claim a contract-to-skill result is a new live skill without checking repo context when repo files are available.
...
## V003
- Added repo-backed manifest/index checking so contract-derived skills can be classified as new skills versus amendments.
- Added a CTS-style one-question clarification gate for identity, authority, target runtime, and destructive scope.
- Added explicit boundaries against overwriting live skills or generating unrequested companion artifacts.
